...
class RobotFFModel(nn.Module):
    def __init__(self, inp_dim, out_dim, n_layers, size, activation='relu'):
        super().__init__()
    ...
    def forward(self, state, acs, data_stats):
        state_normalized = normalize(state, data_stats['state_mean'], data_stats['state_std'])
        acs_normalized = normalize(acs, data_stats['acs_mean'], data_stats['acs_std'])

        input = torch.cat([torch.from_numpy(state_normalized),torch.from_numpy(acs_normalized)], dim=1)
        #input = torch.from_numpy(acs_normalized)

        pred_normalized = self.state_network(input)
        next_state = state + unnormalize(pred_normalized.detach().numpy(), data_stats['delta_mean'], data_stats['delta_std'])
...
class NeuralStateSpaceModel(nn.Module):
    ...
    def __init__(self, n_x, n_u, n_feat=64, scale_dx=1.0, init_small=True, activation='tanh'):
